...
export async function POST(req: NextRequest) {
  try {
    ...
    // The system prompt instructs Claude to spread >=8 questions across >=4 curriculum
    // days; question count is the reliable signal we can gate on client/server-side
    // without adding structured per-turn output that would slow the live loop down.
